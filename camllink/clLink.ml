...
module QIdentSet = Set.Make(struct
    type t = qualified_ident
    let compare = compare_qualids
  end)

let missing_globals = ref QIdentSet.empty

let is_required = function
    Reloc_setglobal id, _ -> QIdentSet.mem id !missing_globals
  | _ -> false;;

let remove_required = function
    Reloc_setglobal id, _ -> missing_globals := QIdentSet.remove id !missing_globals
  | _ -> ();;

let add_required = function
    Reloc_getglobal id, _ -> missing_globals := QIdentSet.add id !missing_globals
  | _ -> ();;
...
let require_qualid qual id =
  missing_globals := QIdentSet.add {qual=qual; id=id} !missing_globals;;
...
let link_object outchan (truename, required) =
  ...
  try
    ...
    close_in inchan
  with x ->
    eprintf "Error while linking file %s.\n" truename;
    close_in inchan;
    raise x
;;
...
let emit_data outstream =
  let globals = make_vect (number_of_globals()) (Obj.repr 0) in
  do_list
