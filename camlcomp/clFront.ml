...
let translate_compar gen_fun (int_comp, float_comp) ty arg1 arg2 =
  let comparison =
    if ClTypes.same_base_type ty type_int ||
       ClTypes.same_base_type ty type_char then
      Ptest int_comp
    else if ClTypes.same_base_type ty type_float then
      Ptest float_comp
...
let rec translate_expr env =
  ...
  | Zlet(true, pat_expr_list, body) ->
      ...
      Lletrec(map translate_rec_bind pat_expr_list,
              ClEvent.before new_env body (translate_expr new_env body))
  | Zfunction [] ->
  ...
  | Zfunction((patl1,act1)::_ as case_list) ->
      let rec transl_fun debug_env = function
        ...
        | pat::patl ->
            ...
            Lfunction(ClEvent.after_pat new_debug_env pat
  ...
  | Zsequence(e1, e2) ->
      Lsequence(transl e1, ClEvent.before env e2 (transl e2))
  | Zcondition(eif, ethen, eelse) ->
      Lifthenelse(transl eif,
                  ClEvent.before env ethen (transl ethen),
                  if match eelse.e_desc with
                       Zconstruct0(cstr) -> cstr == constr_void | _ -> false
                  then transl eelse
                  else ClEvent.before env eelse (transl eelse))
  | Zwhile(econd, ebody) ->
      Lwhile(transl econd, ClEvent.before env ebody (transl ebody))
  | Zfor(id, estart, estop, up_flag, ebody) ->
      let new_env = add_for_parameter_to_env env id in
      Lfor(transl estart,
           translate_expr (Treserved env) estop,
           up_flag,
           ClEvent.before new_env ebody (translate_expr new_env ebody))
  | Zconstraint(e, _) ->
      transl e
  | Zvector [] ->
  ...
  | Zparser case_list ->
      let (stream_type, _) = ClTypes.filter_arrow expr.e_typ in
...
and transl_action env (patlist, expr) =
  ...
  (patlist, add_lets(ClEvent.before new_env expr action))
