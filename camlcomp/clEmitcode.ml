...
let out_int_const i =
  if i <= (maxint_byte-1)/2 && i >= (minint_byte-1)/2 then begin
    ...
    out opGETGLOBAL; ClReloc.slot_for_literal(SCatom(ACint i))
  end
;;
...
let out_tag = function
  ...
  | ConstrExtensible(name, stamp) ->
      ClReloc.slot_for_tag name stamp
;;
...
let rec emit = function
    ...
    | Kquote(SCblock(tag,[])) :: code ->
        ...
        | ConstrExtensible(name, stamp) ->
            out opATOM; ClReloc.slot_for_tag name stamp
        end;
        emit code
    | Kquote(sc) :: code ->
        out opGETGLOBAL;
        ClReloc.slot_for_literal sc;
        emit code
    | Kget_global qualid :: code ->
        out opGETGLOBAL;
        ClReloc.slot_for_get_global qualid;
        emit code
    | Kset_global qualid :: code ->
        out opSETGLOBAL;
        ClReloc.slot_for_set_global qualid;
        emit code
    | Kaccess n :: code ->
    ...
    | Ktest(tst,lbl) :: code ->
        begin match tst with
          ...
          | Pfloat_test(PTnoteqimm f) ->
              ...
              ClReloc.slot_for_literal (SCatom(ACfloat f));
          ...
          | Pstring_test(PTnoteqimm s) ->
              out opPUSH; out opPUSH; out opGETGLOBAL;
              ClReloc.slot_for_literal (SCatom(ACstring s));
