...
let module_table = (hashtbl__new 37 : (string, mod_type) hashtbl__t);;
...
let read_module basename filename =
  ...
  try
    let md = (input_value ic : mod_type) in
    close_in ic;
    md.mod_persistent <- true;
    md
  with End_of_file | Failure _ ->
    close_in ic;
    eprintf "Corrupted compiled interface file %s.\n\
...
let load_module name =
  ...
  with Cannot_find_file _ ->
    eprintf "Cannot find the compiled interface file %s.zi.\n" name;
    raise Toplevel
;;
...
let open_module name =
  let mod_type = find_module name in
  add_table mod_type.mod_values (!opened_modules).mod_values;
  add_table mod_type.mod_constrs (!opened_modules).mod_constrs;
  add_table mod_type.mod_labels (!opened_modules).mod_labels;
  add_table mod_type.mod_types (!opened_modules).mod_types;
  opened_modules_names := name :: !opened_modules_names;
  hashtbl__add !used_opened_modules name (ref false);;

(* Close a mod_type and remove its definitions from the table of opened modules.
...
(* Additions to the mod_type being compiled *)
...
(* Find the descriptor for a reference to a global identifier.
   If the identifier is qualified (mod__name), just look into mod_type mod.
  ...
  | GRname s ->
      ...
      with Not_found ->
        try
          ...
          (* Record the mod_type as actually used *)
...
(* To write the interface of the mod_type currently compiled *)
